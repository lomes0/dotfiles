...
vim.api.nvim_create_user_command("LuaProfileStop", function()
	...
	-- build top lists
	local arr = {}
	for k, ns in pairs(selftime) do
		table.insert(arr, { k = k, ns = ns })
	end
	table.sort(arr, function(a, b)
		return (a.ns or 0) > (b.ns or 0)
	end)
	local out = {}
	local function ms(ns)
		return string.format("%.2f", (ns or 0) / 1e6)
	end
	...
	local files = {}
	for f, ns in pairs(perfile) do
		table.insert(files, { f = f, ns = ns })
	end
	...
	local f = assert(io.open(PROF_PATH, "w"))
